...
describe User do
  ...
  describe '.find_or_create_with_twitter_auth_hash' do
    subject(:find_or_create_with_twitter_auth_hash) do
      User.find_or_create_with_twitter_auth_hash(auth_hash)
    end
    ...
    context 'when user does not exist' do
      ...
      it 'creates user' do
        expect { find_or_create_with_twitter_auth_hash }.to change(User, :count).by(1)
      end
    end
    ...
    context 'when user exists' do
      ...
      it 'does not create user' do
        expect { find_or_create_with_twitter_auth_hash }.not_to change(User, :count)
      end
    end
  end
end
